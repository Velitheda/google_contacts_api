...
describe GoogleContactsApi::ContactsAccessor do
    ...
    describe ".get_contacts" do
      it "should get the contacts using the internal @api object" do
        ...
        expect(stubbed_contacts_accessor_class.new(api).get_contacts).to eq("contact set")
      end
    end
    ...
    describe ".put_or_post_contact" do
      it "should put the contact when it exists" do
        ...
        expect(stubbed_contacts_accessor).to receive(:put_contact).with(@contact).and_return(200)
        expect(stubbed_contacts_accessor.put_or_post_contact(@contact))#.to #eq(200)
      end

      it "should post the contact when it doesn't exist" do
        stubbed_contacts_accessor = stubbed_contacts_accessor_class.new(@api)
        expect(stubbed_contacts_accessor).to receive(:put_contact).with(@contact).and_return(404)
        expect(stubbed_contacts_accessor).to receive(:post_contact).with(@contact).and_return(201)
        expect(stubbed_contacts_accessor.put_or_post_contact(@contact)).to eq(201)
      end
    end
    ...
    describe ".post_contact" do
      it "should post the contact using the internal @api object" do
        ...
        expect(stubbed_contacts_accessor_class.new(api).post_contact(@contact)).to eq(201)
      end
    end
    ...
    describe ".put_contact" do
      it "should put the contact using the internal @api object" do
        ...
        expect(stubbed_contacts_accessor_class.new(api).put_contact(@contact)).to eq(200)
      end
    end
